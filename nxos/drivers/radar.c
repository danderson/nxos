...
static bool radar_read(U8 sensor, radar_memory_slot slot, U8 *val) {
  ...
  return i2c_memory_read(sensor, cmd->addr, val, cmd->len) == I2C_ERR_OK;
}
...
static bool radar_write(U8 sensor, radar_memory_slot slot, U8 *val) {
  struct radar_cmd_info *cmd = &radar_cmds[slot];
  return i2c_memory_write(sensor, cmd->addr, val, cmd->len) == I2C_ERR_OK;
}
...
bool radar_read_all(U8 sensor, U8 *buf) {
   ...
   */
  return i2c_memory_read(sensor, radar_cmds[RADAR_R0].addr,
                         buf, 8) == I2C_ERR_OK;
}
...
void radar_info(U8 sensor) {
  ...
  // Product ID (LEGO)
  memset(buf, 0, 8);
  radar_get_product_id(sensor, buf);
  display_string((char *)buf);
  display_string(" ");

  // Sensor Type (Sonar)
  memset(buf, 0, 8);
  radar_get_sensor_type(sensor, buf);
  display_string((char *)buf);
  display_string(" ");
  
  // Version (V1.0)
  memset(buf, 0, 8);
  radar_get_version(sensor, buf);
  display_string((char *)buf);
  display_end_line();
  ...
  memset(buf, 0, 8);
  radar_get_measurement_units(sensor, buf);
  display_string((char *)buf);
  display_end_line();
  
  // Measurement interval
  display_string("Interval: ");
  display_uint(radar_get_interval(sensor));
  display_string(" ms?\n");
}
